...
## Deploy boundary controller and worker
```sh
...
# Update `public_addr` and `initial_upstreams` in `boundary-worker.hcl` file based on the above output
...
## Configure OCP as Boundary target
```sh
